...
import { AUTH_OTP_LENGTH, OtpInput, OtpResendRow } from '../components/OtpInput';
...
export function OtpVerifyScreen(): React.ReactElement {
  ...
  }, [country, dispatch, navigation, otp, phone]);
  ...
  const onResend = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        Alert.alert('', result.message ?? result.error ?? 'Unable to resend code.');
      }
    } finally {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }} edges={['top', 'bottom']}>
      ...
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: spacing.xl }}>
        ...
        <OtpResendRow onResend={() => void onResend()} loading={resendLoading} />
      </ScrollView>
    </SafeAreaView>
  );
}
